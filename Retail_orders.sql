...
with cte as (
select region, product_id,  sum(sale_price) as sales_region
from df_orders
group by region, product_id )
select * from (
select * , ROW_NUMBER() over(partition by region order by sales_region desc) as rn
from cte ) A
where rn <= 5;
